# DDD Specification Templates for SpecFrame
...
4. Gaps are written into the feature spec first, then resolved by the human — same gap resolution flow as any SpecFrame specification
...
The domain model — Product, BacklogItem, Sprint, Discussion, and the three Bounded Contexts (Agile PM, Collaboration, Identity & Access) — originates from Vernon's work. The specification format, SpecFrame integration, feature-level extraction pattern, and agentic development workflow are original contributions.
